...
1. `replace(/[^\w\s]/gi, '').toLowerCase()` elimina los signos de puntuación y convierte el texto a minúsculas.
...
2. `split(/\s+/)` divide el texto en palabras utilizando espacios en blanco como delimitadores.
    Divide el texto en un arreglo de palabras.
    La expresión regular \s+ significa "uno o más espacios en blanco" (incluye espacios, tabulaciones y saltos de línea).
3. Un bucle `for` cuenta la frecuencia de cada palabra.
   const recuento = {}:
...
4. Finalmente, se muestra el recuento de cada palabra en la consola.
Object.entries(recuento):
Convierte el objeto recuento en un arreglo de pares [clave, valor] (por ejemplo, [["hola", 2], ["cómo", 1], ["estás", 1]]).
for (const [palabra, cantidad] of Object.entries(recuento)):
